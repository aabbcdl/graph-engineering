...
The graph is explicit opt-in, not automatic overhead. Start a new run only when the user names Graph Engineering in the current task or explicitly confirms a concrete Graph recommendation. Open-ended audit-and-fix work, multiple independent specialties, long-running work, or independent verification may justify recommending Graph, but none of them authorizes a run by itself. Approval never carries across tasks and is recorded before run state is created.
...
New version 2 runs default to `auto`. A Git workspace is reproduced in a dedicated detached worktree, including dirty tracked changes and untracked files captured at launch. A non-Git workspace is copied without following links. The source workspace remains the run identity; every node operates only in the frozen execution workspace.

Source changes after launch do not invalidate an isolated run. Final output is exported as a result package containing source records, result records, changed files, and a local apply script. The runner never merges automatically. The apply script validates path containment, linked parents, and every launch-time source fingerprint before making any change; one conflict refuses the entire apply. `purge` removes an inactive run's registered worktree and evidence without touching the source workspace.
...
Role-specific preflight budgets cap every non-planner prompt. Dependency results and proof are reduced to the fields needed by the consumer, local Skill paths are replaced by logical paths, and command outputs are represented by status and hashes rather than repeated text. A prompt that still exceeds its budget stops with `NODE_INPUT_BUDGET_EXCEEDED` before a model process starts and is not blindly retried.
...
Authorization is a structured state transition, never a keyword inference. Planning may open a gate only through `owner_gate.required=true` with one concrete protected action and exact unblock scope. Synthesis may open a later gate only through a structured `AUTHORIZATION` or `OWNER_GATE` blocker with `required_for_current_goal=true` and an exact protected action. `false` records optional, excluded, or safely deferred work without blocking reversible repository changes. A missing decision is a synthesis correction, not permission to guess. Stage supervision must reject a missing or false required gate before the next phase. Goal text, scope labels, findings, recommendations, quoted source, and negated prose are evidence for those agents but cannot directly change runner state. This keeps review of authentication, payments, production, release, or data surfaces distinct from performing a protected mutation. Observed prohibited commands and Git-state changes remain independent hard stops even when an agent reports no gate.
